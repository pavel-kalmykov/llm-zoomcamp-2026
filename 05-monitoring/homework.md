...
> `DEEPSEEK_API_KEY` was already available. `rag_helper.py`'s `llm()` uses
> `chat.completions.create` instead of `responses.create` (DeepSeek doesn't
> support the Responses API), and `rag()` reads
...
from traced import RAGTraced

rag = RAGTraced(index=index, llm_client=client, model="deepseek-chat")
rag.rag("How does the agentic loop keep calling the model until it stops?")
```

```
3 ReadableSpan entries printed: "search", "llm", "rag"
```
...
```
input_tokens: 7288  (identical across 4 separate runs, see Q6)
```
...
```
llm span durations across 3 separate runs (console exporter start_time/end_time):
run 1: 5096ms
run 2: 4274ms
run 3: 3651ms

search span duration: ~1ms (in-memory minsearch, near-instant)
```
...
con.execute("SELECT DISTINCT name FROM spans").fetchall()
```
...
**Answer:** `llm` (about 3 orders of magnitude more time than `search`)
...
```
[7288, 7288, 7288, 7288]  (4 runs, 0% variation)
```

**Answer:** They're identical

`minsearch` is deterministic TF-IDF over a fixed index, so the same query
always retrieves the same 5 documents, and the resulting context (and its
token count) never changes across runs.
