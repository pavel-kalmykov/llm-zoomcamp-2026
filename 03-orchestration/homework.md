...
Same kind of model; the difference is that the Copilot retrieves from current
Kestra plugin docs (RAG over the docs), so it knows the up-to-date task types
and fields.
...
Executed both flows (both SUCCESS). `1_chat_without_rag.textOutput` hallucinates
plausible-but-wrong Kestra 1.1 features (Declarative Python Tasks, an LDAP user
store, GCS as internal storage, ...); `2_chat_with_rag.textOutput` grounds the
answer in the ingested release notes and lists real ones (New Filters, No-Code
Dashboard Editor, Multi-Agent AI, ...).
...
Executed `4_simple_agent` with `summary_length=short` (SUCCESS). The
`multilingual_agent` output token count for this run was 133; the expected range
for a 1-2 sentence summary is 60-100, which is the closest option.
...
Executed the modified flow (`english_brevity` asking for 3 sentences) with
`summary_length=long` (SUCCESS). `english_brevity` output tokens: 3 sentences=88
vs 1 sentence=41 (ratio ~2.15x).
...
Agents trade determinism for flexibility. For compliance-heavy production work
you want repeatability and auditability, which deterministic task-based workflows
give you and LLM agents do not guarantee.
